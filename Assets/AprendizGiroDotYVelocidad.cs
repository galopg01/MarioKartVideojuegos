...
public class AprendizGiroDotYVelocidad : MonoBehaviour
{
    ...
    IEnumerator Entrenamiento()
    {
        ...
        if (casosEntrenamiento.numInstances() < 10)
        {
            ...
            for (float velocidad = 0; velocidad <= 8.1; velocidad = velocidad + 1)
            {
                for (float rotation = 0; rotation <= 181; rotation = rotation + 10)
                {
                    for (float aceleracion = 0; aceleracion <= 1.05; aceleracion = aceleracion + 0.2f)
                    {
                        for (float giro = -1; giro <= 1.05; giro = giro + 0.2f)                    //Bucle de planificaci�n de la fuerza FY durante el entrenamiento
                        {
                            ...
                            wheelController.speed = aceleracion;
                            ...
                            print("ENTRENAMIENTO: con velocidad " + velocidad + " y dotInicial " + DotInicial + " y aceleraci�n " + aceleracion + " y giro " + giro + " se alcanz� dotFinal de " + script.dot);
            ...
            /*File salida = new File("Assets/Finales_Experiencias_GiroDotYVelocidad.arff");
            if (!salida.exists())
        ...
        saberPredecirDotFinal = new M5P();                                                //crea un algoritmo de aprendizaje M5P (�rboles de regresi�n)
        ...
        if ((ESTADO == "Con conocimiento") && Time.time > 0.5)
        {
            ...
            if (esperando && Time.time - time >= 0.5)
            {
                for (float aceleracion = 0; aceleracion <= 1.05; aceleracion = aceleracion + 0.2f)                                           //Bucle FOR con fuerza Fy, deduce Fx = f (Fy, distancia) y escoge mejor combinacion         
                {
                    ...
                    if (Mathf.Abs(dotFinal - distanciaObjetivo) < menorDistancia || Mathf.Abs(dotFinal) - menorDistancia < 0.02)                     //Busca la Fy con una distancia m�s cercana al objetivo
                    {
                        ...
                        //print("RAZONAMIENTO: Para dotInicial" + script.dot + " y velocidad " + r.velocity.magnitude + " , una posible acci�n es ejercer una aceleraci�n de = " + mejorAceleracion + " y giro = " + mejorGiro + " se alcanzar�a una distancia de " + dotFinal);
                    }
                }
                ...
                print("DECISION REALIZADA: Se ejerci� aceleraci�n " + mejorAceleracion + " y giro " + mejorGiro);
                esperando = false;
            }
        }
